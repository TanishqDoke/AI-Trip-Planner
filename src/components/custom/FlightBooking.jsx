...
function FlightBooking({ trip }) {
    ...
    useEffect(() => {
        if (searchParams.departureDate && noOfDays) {
            const departDate = new Date(searchParams.departureDate);
            const returnDate = new Date(departDate);
            returnDate.setDate(departDate.getDate() + parseInt(noOfDays));
            setSearchParams(prev => ({
                ...prev,
                returnDate: returnDate.toISOString().split('T')[0]
            }));
        }
    }, [searchParams.departureDate, noOfDays]);
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                toast.success(`Found ${result.flights?.length || 0} flights`);
        ...
        <div className='bg-white rounded-2xl border border-slate-200 shadow-sm p-6 mt-8'>
            <div className='flex items-center justify-between mb-6'>
                <div className='flex items-center gap-3'>
                    ...
                    <div>
                        <h2 className='text-xl font-semibold text-slate-900'>Book Flights</h2>
                        <p className='text-sm text-slate-600'>Search and book flights for your {noOfDays}-day trip</p>
                    </div>
                </div>
                ...
                <div className='flex items-center gap-2'>
                    ...
                    <span className='text-xs text-slate-600'>
                        {backendStatus === 'online' ? 'Booking Service Online' :
                            backendStatus === 'offline' ? 'Booking Service Offline' :
                                'Checking...'}
                    </span>
                </div>
            </div>
                    ...
                    <div className='flex items-start gap-3'>
                        ...
                        <div>
                            ...
                            <p className='text-xs text-yellow-700 mt-1'>
                                To use flight booking, start the Python backend server:
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-6'>
                <div>
                    <label className='text-sm font-medium text-slate-700 mb-2 block'>
                        From (Origin Airport Code) <span className='text-red-500'>*</span>
                    </label>
                    <Input
                        placeholder='e.g., BOM for Mumbai, DEL for Delhi'
                        value={searchParams.origin}
                        onChange={(e) => setSearchParams({ ...searchParams, origin: e.target.value.toUpperCase() })}
                        className='uppercase'
                    />
                    <p className='text-xs text-slate-500 mt-1'>Enter 3-letter airport code</p>
                </div>
                <div>
                    <label className='text-sm font-medium text-slate-700 mb-2 block'>
                ...
                <div>
                    ...
                    />
                    <p className='text-xs text-slate-500 mt-1'>Based on {noOfDays}-day trip</p>
                </div>
                <div>
            ...
            {flights.length > 0 && (
                <div className='mt-6 space-y-4'>
                    <div className='flex items-center justify-between'>
                        <h3 className='font-semibold text-slate-900'>Available Flights ({flights.length})</h3>
                        <span className='text-xs text-slate-600'>Powered by Google ADK + Amadeus</span>
                    </div>
                            ...
                            <div className='flex flex-col md:flex-row justify-between gap-4'>
                                <div className='flex-1'>
                                    <div className='flex items-center gap-2 mb-2'>
                                        ...
                                        <span className={`px-2 py-1 rounded text-xs font-medium ${flight.stops === 0 ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                                            }`}>
                                    ...
                                    <div className='grid grid-cols-2 gap-4 text-sm'>
                                        <div>
                                            ...
                                            <p className='font-semibold text-slate-900'>{flight.departure.airport}</p>
                                            <p className='text-slate-600'>{new Date(flight.departure.time).toLocaleString('en-IN', {
                                                dateStyle: 'short',
                                                timeStyle: 'short'
                                            })}</p>
                                        </div>
                                        <div>
                                            ...
                                            <p className='font-semibold text-slate-900'>{flight.arrival.airport}</p>
                                            <p className='text-slate-600'>{new Date(flight.arrival.time).toLocaleString('en-IN', {
                                                dateStyle: 'short',
                                                timeStyle: 'short'
                                            })}</p>
                                        </div>
